...
class TestUrlEncoding(TestBase):
    def test_it(self):
        for test_case in ENCODING_TEST_CASES:
            ...
            encoded = b2_sdk.utils.b2_url_encode(string)
            ...
            self.assertEqual(string, b2_sdk.utils.b2_url_decode(fully_encoded))
            self.assertEqual(string, b2_sdk.utils.b2_url_decode(minimally_encoded))
...
class TestChooseParts(TestBase):
    def test_it(self):
        ...
        actual = b2_sdk.utils.choose_part_ranges(ten_TB, 100 * 1000 * 1000)
        self.assertEqual(expected, actual)

    def _check_one(self, expected, content_length, min_part_size):
        self.assertEqual(expected, b2_sdk.utils.choose_part_ranges(content_length, min_part_size))
...
class TestFormatAndScaleNumber(TestBase):
    ...
    def _check_one(self, expected, x):
        self.assertEqual(expected, b2_sdk.utils.format_and_scale_number(x, 'B'))
